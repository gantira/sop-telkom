  ...
  <section class="section">
    ...
    <nav class="level">
      ...
      <div class="level-left">
        <div class="level-item">
          <p class="subtitle is-5">
            <strong>{{ publishes.length }}</strong> publishes
          </p>
        </div>
        <div class="level-item">
            ...
            <div class="field has-addons">
              <p class="control">
                <input
                  ...
                  type="text"
                  placeholder="Find a publish type"
                />
              </p>
              <p class="control">
      ...
      <div class="level-right">
        ...
        <nuxt-link class="level-item button is-success" :to="{name: 'admin-publishes-create'}">New</nuxt-link>
      </div>
    </nav>
        ...
        <template v-if="selected">
          <nav class="level">
                ...
                <b-field grouped group-multiline>
                  <div class="control">
                    <b-taglist attached>
                      ...
                      <b-tag type="is-info">{{ selected.type }}</b-tag>
                    </b-taglist>
                  </div>
                  <div class="control">
                    <b-taglist attached>
                      <b-tag type="is-dark">Description</b-tag>
                      <b-tag type="is-info">{{ selected.description }}</b-tag>
                    </b-taglist>
                  </div>
                </b-field>
              </div>
            ...
            <div class="level-right">
              <div class="level-item">
                <nuxt-link
                  :to="{name: 'admin-publishes-id-edit', params: {id: selected.id}}"
...
export default {
  ...
  computed: {
    ...mapGetters("admin/publishes", ["publishes", "isLoading"])
  },
  ...
  methods: {
    ...
    async destroy() {
      this.$buefy.dialog.confirm({
        title: "Deleting publish type",
        message:
          "Apakah Anda yakin ingin <b> menghapus </b> type publish ini? Tindakan ini tidak bisa dibatalkan.",
        confirmText: "Delete publish type",
        type: "is-danger",
        hasIcon: true,
        onConfirm: () => {
          for (const item of this.checkedRows) {
            this.$store.dispatch("admin/publishes/delete", item.id);
          }
          this.checkedRows = [];
          this.$buefy.toast.open("Publish type deleted!");
        }
      });
    },
    find() {
      this.$store.dispatch("admin/publishes/get", this.search);
    }
  }
};
</script>
