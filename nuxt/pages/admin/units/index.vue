  ...
  <section class="section">
    ...
    <nav class="level">
      ...
      <div class="level-right">
        ...
        <nuxt-link class="level-item button is-success" :to="{name: 'admin-units-create'}">New</nuxt-link>
      </div>
    </nav>
        ...
        <template v-if="selected">
          <nav class="level">
            ...
            <div class="level-right">
              <div class="level-item">
                <nuxt-link
                  :to="{name: 'admin-units-id-edit', params: {id: selected.id}}"
  ...
  methods: {
    ...
    async destroy() {
      this.$buefy.dialog.confirm({
        title: "Deleting unit",
        message:
          "Apakah Anda yakin ingin <b> menghapus </b> unit? Tindakan ini tidak bisa dibatalkan.",
        confirmText: "Delete unit",
        type: "is-danger",
        hasIcon: true,
        onConfirm: () => {
          for (const item of this.checkedRows) {
            this.$store.dispatch("admin/units/delete", item.id);
          }
          this.checkedRows = [];
          this.$buefy.toast.open("unit deleted!");
        }
      });
    },
    find() {
      this.$store.dispatch("admin/units/get", this.search);
    }
  }
};
</script>
