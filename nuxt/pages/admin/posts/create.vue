  ...
  <div>
    ...
    <section>
      <form @submit.prevent="add">
        ...
        <b-button native-type="submit" type="is-primary">Create a post</b-button>
      </form>
    </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    add() {
      this.$axios.post("admin/posts", this.form).then(response => {
        this.$router.push({ name: "admin-posts" });

        this.$buefy.notification.open({
          duration: 5000,
          message: `Anda baru saja memposting "${this.form.title}".`,
